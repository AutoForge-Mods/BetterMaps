...
prefab:addTransform({})
prefab:addRender("iron_ore")
prefab:addItem({icon = "icons_iron_ore", stack = 100, categories = {"metal", "smeltable"}, materialSound = fmod.MATERIAL_ORE})
prefab:addTile({
    durability = 8,
    priority = 128,
    friction = 0.95,
    drop = "material.iron_ore",
    preview = "iron_ore_preview",
    tags = {"natural", "ore"},
    materialSound = fmod.MATERIAL_ORE,
    terrainSound = fmod.TERRAIN_ORE,
    harvestSound = fmod.HarvestTile.Stone,
    mapColor = Color.new(50, 80, 140),
    mapBackwallColor = Color.new(50, 80, 150)
})
